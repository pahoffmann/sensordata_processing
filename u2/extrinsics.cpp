...
float _size_of_square = 3.9f;

//left
float _hor_offset_l = 1.5f;
float _vert_offset_l = 1.05f;

//right
float _hor_offset_r = 1.5f;
float _vert_offset_r = 1.05f;
...
{
    // define the world coords of my schachbrett alla -> using two schachbretts alla
    

    // left (up left to down right)
    for(int i = _num_corners_vert ; i >= 1; i--)
    {
        for(int j = _num_corners_hor ; j >= 1; j--)
        {
    ...
    // first right chessboard (down right to up-left)
    ...
    // old stuff, used fpr shitty shit 
...
void calculateExtrinsics()
{
    ...
    while (found_counter != 2) {
        ...
        {
            std::cerr << "Webcam and or chessboard not positioned correctly!" << std::endl;
            //return;
        }
...
void saveExtrinsics()
{
    std::cout << "Saving extrinsics..." << std::endl;
    /* save camera params */
    cv::FileStorage fs("extrinsicParams.yaml", cv::FileStorage::WRITE);
