...
static void StereoCalib(VideoCapture& capture1, VideoCapture& capture2, Size boardSize, float squareSize, bool displayCorners = false, bool useCalibrated=true, bool showRectified=true)
{
    ...
    imagePoints[0].resize(MAX_IMAGES);
    imagePoints[1].resize(MAX_IMAGES);
    ...
    while(j < MAX_IMAGES)
    {
        std::cout << "Bild-Tupel Nr.: " << j << std::endl;

        Mat img1, img2, gray_1, gray_2;
        ...
        cv::cvtColor(img1, gray_1, cv::COLOR_RGB2GRAY);
        cv::cvtColor(img2, gray_2, cv::COLOR_RGB2GRAY);
        ...
        for(k = 0; k < 2; k++ )
        {
            ...
            if( displayCorners )
            {
                ...
                std::cout << imagePoints[k].size() << std::endl;
        ...
        if( k == 2 )
        {
            goodImageList.push_back(img1);
            goodImageList.push_back(img2);
            j++;
        }
    }

    imagePoints[0].resize(j);
    imagePoints[1].resize(j);
    objectPoints.resize(j);

    for(int  i = 0; i < j; i++ )
    {
        for(int f = 0; f < boardSize.height; f++ )
...
int main(int argc, char** argv)
{
    ...
    */
    ...
    if (!camera1.isOpened()) {
        std::cerr << "ERROR: Could not open camera 1" << std::endl;
        return -1;
    }

    if (!camera2.isOpened()) {
        std::cerr << "ERROR: Could not open camera 2" << std::endl;
        return -1;
    }   
    ...
    */
    ...
    StereoCalib(camera1, camera2, boardSize, squareSize, true, true, showRectified);
